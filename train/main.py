...
def train(args, model, enc=False):
    ...
    # weight==[2.74229466922693,3.91480285813438,3.72231850597399,2.44085007708424,1.59291882175928,1.26241291004510,0.911728038745930,1,0.973876438071609,0.994387709113245,1.10524458606599,0.906882258442405,0.886658413615704,0.887996668176704,0.907523911567148,0.943862722223145,1.44305225656323,0.0633530449186512]
    #real=[1.69795652615487	1.32366781703028	.27736085039098	1.20109826871480	1.10698501596935	1.05613926080263	0.994327308698008	0.928366909112762	0.890972968746974	0.907599703514519	0.902284868237001	0.912360448491215	0.931554174584365	0.966006176079838	1.00573778871021	1.08564699411410	3.50887022554180	0.0690765203794107]
    #zer=[2.27226822774319	1.69117589533845	1.29345405576351	1	0.799415172153159	0.659845544704968	0.535968297939808	0.364784703146847	0.360890107911680	0.537723086457573	0.674155269881703	0.834986553669595	1.06156385341097	1.41324794424111	1.95190771208442	2.68519805272189	2082.33034294077]
    #gaus=[1.15831299845958	1.33864102529081	1.17662622593310	1.02939026612044	0.916575261416261	0.834550180851512	0.781508324986013	0.730528344125685	0.726540019672428	0.777018135053042	0.815826474746400	0.896071720297729	1	1.14460421341049	1.32073750743811	1.14556792510438	78.3520625185478]
    #simu=[2.75003337880663	3.92585036616510	3.73282282639007	2.44773811507889	1.59741401196767	1.26597541811184	0.914300919985389	1.00282198323417	0.976624701051996	0.997193854556623	1.10836356775752	0.909441464971091	0.889160548793359	0.890502579886293	0.910084928830194	0.946526287000612	1.44712452583728	0.0633530449186512]
    # if (enc):
    # # gaus:
    #     weight[0]=1.1583
    #     weight[1]=1.3386
    ...
    loader = DataLoader(dataset_train, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False,sampler=loader_sample)
    loader_val_sample = RandomSampler(dataset_val, replacement=True, num_samples=1000, generator=None)
    loader_val = DataLoader(dataset_val, num_workers=args.num_workers, batch_size=args.batch_size, shuffle=False,sampler=loader_val_sample)
    ...
    if (not os.path.exists(automated_log_path)):    #dont add first line if it exists 
        with open(automated_log_path, "a") as myfile:
            myfile.write("Epoch\t\tTrain-loss\t\tTest-loss\t\tcre_val\t\tssim_val\t\trmse_val\t\tTrain-IoU\t\tTest-IoU\t\tlearningRate")
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        save_checkpointloss({
            ...
            'best_rmse':best_rmse,
            'optimizer': optimizer.state_dict(),
        }, is_best_loss,is_best_ssim,is_best_cre,is_best_rmse, filenameCheckpointloss, filenameBestloss,filenameBestssim,filenameBestrmse,filenameBestcre)
        ...
        with open(automated_log_path, "a") as myfile:
            myfile.write("\n%d\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.8f" % (epoch, average_epoch_loss_train, average_epoch_loss_val,average_epoch_cre_val,average_epoch_ssim_val,average_epoch_rmse_val, iouTrain, iouVal, usedLr ))
...
def save_checkpointloss(state, is_best_loss,is_best_ssim,is_best_cre,is_best_rmse, filenameCheckpointloss, filenameBestloss,filenameBestssim,filenameBestrmse,filenameBestcre):
...
def main(args):
    ...
    # model = model_file.DeepLabv3_plus(NUM_CLASSES)
    ...
    if (not args.state):
        ...
        model = model_file.EESANet(imagechan=1,chann=48,numclass=NUM_CLASSES)
        # model = model_file.Net(NUM_CLASSES)
        # model = model_file.DeepLabv3_plus()
        #model = model_file.frrn()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="EESANet")
    ...
    parser.add_argument('--savedir', default="D:/2022_1_5_work/myloss/new_blender//blender/100blender/result/")
    ...
    parser.add_argument('--onlycre', action='store_true')
