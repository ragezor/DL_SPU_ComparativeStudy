...
from transform import Relabel, ToLabel, Colorize, Colorizeblender, Colorizeerror
...
def main(args):
    ...
    for step, (images, labels, filename, filenameGt) in enumerate(loader):
        ...
        sub=abs(label-labels)
        # abscha=abs(cha)

        label_color = Colorize()(label.unsqueeze(0))
        gt_color=Colorize()(labels.unsqueeze(0))
        sub_color=Colorize()(sub.unsqueeze(0))
        # cha_color=Colorize()(abscha.unsqueeze(0))
        # newlabel_color=Colorize()(new_label.unsqueeze(0))
        metric="cre"
        filenameSave = "D:/simu_data/our_niose/gaus/"+metric+"/pre/" +filename[0].split("input/")[1][:-4]+"pre_"+metric+".png"
        gtSave = "D:/simu_data/our_niose/gaus/"+metric+"/gt/" +filename[0].split("input/")[1][:-4]+"gt_"+metric+".png"
        subSave = "D:/simu_data/our_niose/gaus/"+metric+"/sub/" +filename[0].split("input/")[1][:-4]+ "sub_"+metric+".png"
